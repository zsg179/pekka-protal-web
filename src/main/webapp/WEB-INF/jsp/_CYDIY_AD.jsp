...
<div class=item-rank>
	...
	<div class=rank-body>
		<c:forEach items="${CYDIYItemRanking }" var="CYDIY" varStatus="status">
			<c:if test="${status.count <= 3 }">
				<div class=rank-body-item>
					<a class="rank-body-item-link"
						href="http://localhost:8086/item/${CYDIY.id }.html">
						<div class="item-link-img" style="margin-left: -15px;">
							<div class="lazyimg"
								style="background: transparent none repeat scroll 0% 0%;"
								alt="${CYDIY.title }" title="${CYDIY.title }">
								<img class="imgloaded" src="${CYDIY.image }"
									alt="${CYDIY.title }" />
							</div>
						</div>
						...
						<div class="goods-info" style="padding-top: 0px;">
							<div class="goods-rank-ranking"
								style="background-color: rgb(221, 69, 69); left: -15px; color: rgb(255, 255, 255); font-size: 14px;">
								${status.count }</div>
							<div class="goods-rank-name" style="color: rgb(102, 102, 102);">
								${CYDIY.title }</div>
							<div class="goods-rank-price" style="color: rgb(221, 69, 69);">
								<fmt:formatNumber value="${CYDIY.price * 0.01}" type="currency"
									pattern="￥.00" />
							</div>
						</div>
					</a>
