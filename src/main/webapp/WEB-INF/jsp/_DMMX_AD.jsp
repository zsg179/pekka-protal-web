...
<body>
	<div class=item-rank>
		...
		<div class=rank-body>
			<c:forEach items="${DMMXItemRanking }" var="DMMX" varStatus="status">
				<c:if test="${status.count <= 3 }">
					<div class=rank-body-item>
						<a class="rank-body-item-link"
							href="http://localhost:8086/item/${DMMX.id }.html">
							<div class="item-link-img" style="margin-left: -15px;">
								<div class="lazyimg"
									style="background: transparent none repeat scroll 0% 0%;"
									alt="${DMMX.title }" title="${DMMX.title }">
									<img class="imgloaded" src="${DMMX.image }"
										alt="${DMMX.title }" />
								</div>
							</div>
							<div class="goods-info" style="padding-top: 0px;">
								...
								<div class="goods-rank-price" style="color: rgb(221, 69, 69);">
									<fmt:formatNumber value="${DMMX.price * 0.01}" type="currency"
										pattern="￥.00" />
								</div>
							</div>
						</a>
